...
--выдача только после погашения => выдача больше 1

--SELECT * FROM [PRODUCTS]
--	WHERE [PRODUCTS].[PRODUCT_TYPE_ID] = 1 AND ([PRODUCTS].[CLOSE_DATE] > GETDATE() OR [PRODUCTS].[CLOSE_DATE] IS NULL) 		

--SELECT * FROM [RECORDS]	
--	WHERE [RECORDS].[ACC_REF] = 220

SELECT [CLIENTS].* FROM [PRODUCTS] JOIN [ACCOUNTS]
	ON [PRODUCTS].[ID] = [ACCOUNTS].[PRODUCT_REF] JOIN
		(SELECT [RECORDS].[ACC_REF], COUNT([RECORDS].[DT]) AS C FROM [RECORDS]
			WHERE [RECORDS].[DT] = 1 
			GROUP BY [RECORDS].[ACC_REF]
			HAVING COUNT([RECORDS].[DT]) > 1
		) AS SJ
		ON SJ.ACC_REF = [ACCOUNTS].[ID]
	JOIN [CLIENTS]
		ON [CLIENTS].[ID] = [ACCOUNTS].[CLIENT_REF]
	WHERE [PRODUCTS].[PRODUCT_TYPE_ID] = 1 AND ([PRODUCTS].[CLOSE_DATE] > GETDATE() OR [PRODUCTS].[CLOSE_DATE] IS NULL)


--SELECT [RECORDS].[ACC_REF], COUNT([RECORDS].[DT]) FROM [RECORDS]
--	WHERE [RECORDS].[DT] = 1 AND ([RECORDS].[ACC_REF] = 212 OR [RECORDS].[ACC_REF] = 220)
--	GROUP BY [RECORDS].[ACC_REF]
--	HAVING COUNT([RECORDS].[DT]) > 1
